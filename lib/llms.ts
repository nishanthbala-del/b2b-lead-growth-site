...
//   * the business's definitions (lib/canon.ts), generated by the operating system from the
//     vocabulary its own code is checked against (2026-09-18).
//
...
import { canonGroups, termsInGroup } from "./canon.ts";
import {
  audit,
  boundarySentence,
  contractorBoundary,
...
export function llmsTxt(): string {
  ...
  // The business's vocabulary, one definition per term, from the operating system. The
  // /definitions page carries the same text with who decides and what must be on record.
  lines.push("## Definitions", "");
  lines.push(
    `Each term below means one thing everywhere ${brandName} uses it. Full entries: ${siteUrl}/definitions`,
    "",
  );
  for (const g of canonGroups) {
    for (const t of termsInGroup(g.key)) lines.push(`- ${t.label}: ${t.definition}`);
  }
  lines.push("");
